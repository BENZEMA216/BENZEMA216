<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?font=Inter&weight=600&size=30&duration=3000&pause=1000&color=333333&center=true&vCenter=true&width=500&height=45&lines=Dongzhe+Zhu" alt="Typing SVG" />

**AI Product Manager × Builder**

<br>

[![GitHub](https://img.shields.io/badge/BENZEMA216-181717?style=flat&logo=github&logoColor=white)](https://github.com/BENZEMA216)
[![Email](https://img.shields.io/badge/Email-ea4335?style=flat&logo=gmail&logoColor=white)](mailto:BENZEMAZDZ99@gmail.com)
[![Blog](https://img.shields.io/badge/Blog-222?style=flat&logo=hashnode&logoColor=white)](https://benzema216.github.io)
![Views](https://komarev.com/ghpvc/?username=BENZEMA216&color=gray&style=flat&label=views)

</div>

<br>

Currently at **ByteDance**, building the Creative AGENT for [Jimeng AI](https://jimeng.jianying.com) from 0→1.
Previously shipped LLM-powered data products at **Tencent** — **2023 Business Breakthrough Award**.

```
在字节做过成熟的 agent 产品，踩过 agent 产研的所有坑
用 GitHub repo 当 PRD 写，Claude Max 20x 消耗者
Vibe coding 做出完整产品并上线的人
```

<br>

<div align="center">

`Python` `TypeScript` `Go` `Swift` `LLM` `Agent` `RAG` `Claude Code`

</div>

---

#### What I'm building

**[OpenClaw](https://github.com/BENZEMA216/xian-home)** — AI Agent Gateway，让 Agent 能力可发现、可调用、可变现
**[弦 Xian](https://github.com/BENZEMA216/HomForClawd)** — 个人 AI Agent，云端分层记忆系统
**[即梦 Studio](https://github.com/BENZEMA216/dreamina-claude-skills)** — 字节即梦 AI 全链路视频自动化
**[Claude Code Skills](https://github.com/BENZEMA216/creative-think)** — dreamina · music-analyzer · creative-think · self-purify

---

#### Stats

<div align="center">

<img height="150" src="https://github-readme-stats.vercel.app/api?username=BENZEMA216&show_icons=true&hide_border=true&title_color=333&text_color=555&icon_color=888&count_private=true&hide_title=true" />
<img height="150" src="https://github-readme-stats.vercel.app/api/top-langs/?username=BENZEMA216&layout=compact&hide_border=true&title_color=333&text_color=555&langs_count=6&hide_title=true" />

</div>

---

<details>
<summary>Personality Profile</summary>
<br>

**CliftonStrengths** — Ideation · Learner · Input · Positivity · Achiever
**Big Five** — Extraversion 89th · Conscientiousness 72nd · Neuroticism 2nd
**Primal Beliefs** — Improvable 99th · Interconnected 99th · Needs Me 99th

> 对新理念的好奇 → 大量吸收信息 → 形成独特洞察 → 快速付诸行动

</details>

<details>
<summary>Experience</summary>
<br>

**ByteDance** · AI Product Manager · Jianying & Jimeng · 2024 - Present
**Tencent** · Product Specialist / Data PM · Overseas Publishing · 2022 - 2024
**NTU** · M.Sc. Computer Control & Automation · GPA 4.6/5, Top 10%
**Tongji** · B.Eng. Automation

</details>

<br>

<div align="center">
<sub><i>"Build agents that build value."</i></sub>
</div>
